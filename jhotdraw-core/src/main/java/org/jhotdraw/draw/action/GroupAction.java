...
public class GroupAction extends AbstractSelectedAction {
    ...
    private void groupSelection() {
        ...
        fireUndoableEditHappened(createGroupEdit(view, group, ungroupedFigures));
    }
    ...
    private void ungroupSelection() {
        ...
        final LinkedList<Figure> ungroupedFigures = new LinkedList<>(ungroupFigures(view, group));
        fireUndoableEditHappened(createUngroupEdit(view, group, ungroupedFigures));
    }

    private UndoableEdit createGroupEdit(DrawingView view, CompositeFigure group, Collection<Figure> figures) {
        return createUndoableEdit("edit.groupSelection.text",
                () -> groupFigures(view, group, figures),
                () -> ungroupFigures(view, group));
    }

    private UndoableEdit createUngroupEdit(DrawingView view, CompositeFigure group, Collection<Figure> figures) {
        return createUndoableEdit("edit.ungroupSelection.text",
                () -> ungroupFigures(view, group),
                () -> groupFigures(view, group, figures));
    }

    private UndoableEdit createUndoableEdit(String presentationId, Runnable redoOperation, Runnable undoOperation) {
        return new AbstractUndoableEdit() {
            private static final long serialVersionUID = 1L;

            @Override
            public String getPresentationName() {
                return ResourceBundleUtil.getBundle("org.jhotdraw.draw.Labels").getString(presentationId);
            }
